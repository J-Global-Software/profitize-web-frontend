			...
			<div className="max-w-[1120px] mx-auto px-4 sm:px-6">
				...
				<motion.div className="flex flex-wrap justify-center gap-3 sm:gap-4" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.25 }} variants={containerVariants}>
					{INDUSTRIES.map(({ key, icon }) => (
						<motion.div key={key} layout className="group flex items-center gap-2 sm:gap-3 px-5 py-3 sm:px-8 sm:py-5 rounded-full bg-white border border-[#e7ebf3] cursor-pointer transition-all duration-300 hover:-translate-y-[2px] hover:bg-[#f0f4ff] hover:border-primary hover:shadow-[0_4px_12px_rgba(23,84,207,0.08)]">
							<span className="material-symbols-outlined text-primary text-[20px] sm:text-[24px] transition-transform group-hover:scale-110">{icon}</span>
